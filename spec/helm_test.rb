...
task :create_cluster do
  ...
  sh("gcloud container clusters create #{cluster_name} --disk-size=#{disk_size} --num-nodes=#{num_of_nodes} --zone #{region} --machine-type=#{machine_type} --cluster-version=1.9.6-gke.0 || true")
